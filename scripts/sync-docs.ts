...
interface CollectionDef {
  ...
  slug: string
  description?: string
  skripts: string[]  // Directory names
}
...
async function main() {
  ...
  const config: CollectionsConfig = JSON.parse(
  ...
  const allSkriptDirs = new Set<string>()
  ...
  const skriptIdMap = new Map<string, string>()  // dir -> skript.id
...
async function processPages(
  ...
  const pageData: PageData[] = mdFiles.map(filename => {
...
async function processAttachments(
  ...
  for (const filename of files) {
    ...
    // Unique constraint is on [parentId, name, skriptId]
    await prisma.file.upsert({
      where: {
        unique_file_name_per_parent_skript: {
          parentId: null as unknown as string, // Prisma quirk: nullable field in unique constraint
          name: filename,
          skriptId
        }
      },
    ...
    // For now, attachments need to be uploaded manually or via import
