 ...
 *
 * Body is either { step } (marks one onboarding-quest step done) or
 * { dismiss: true } (closes the widget without abandoning progress — steps
 * can still complete silently afterward). Once every step is complete, the
 * reward (trial extension) is granted exactly once. Server-side merge (not a
 * client-sent full blob) so concurrent completions from different origins
 * (dashboard vs. a custom-domain public page) can't clobber each other.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!dismiss && (!step || !QUEST_STEPS.includes(step))) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const state: QuestState = {
        ...
        completedSteps: dismiss ? current.completedSteps : { ...current.completedSteps, [step as QuestStep]: now },
