...
interface PublicSiteLayoutProps {
  ...
  routePrefix?: string // Server-side route prefix (e.g., '/org/eduskript/c' for org content, '/{pageSlug}' for teacher pages). Used to build navigation URLs; the proxy-stripped portion is removed at click time so the link matches the browser-visible path.
  homeUrl?: string // Explicit home/logo target. Defaults to routePrefix. Required when routePrefix is a content prefix that isn't itself a valid route (e.g. '/org/{slug}/c' — pass homeUrl='/org/{slug}' to point at the org root instead).
  hideSidebar?: boolean // Hide sidebar entirely (e.g., exam pages)
}

// Hosts where the proxy rewrites `/foo` → `/org/${PRIMARY_ORG}/foo` by default.
// Everything else is treated as a custom host (teacher domain like
// informatikgarten.ch, DB-resolved org domain like schoola.org) where the
// proxy always prepends the tenant segment based on hostname resolution.
// PRIMARY_ORG must stay in sync with DEFAULT_ORG_SLUG in src/proxy.ts.
const PRIMARY_ORG = 'eduskript'
const PRIMARY_HOSTS = ['eduskript.org', 'www.eduskript.org', 'localhost']

function isPrimaryHost(hostname: string): boolean {
  return PRIMARY_HOSTS.includes(hostname)
    || hostname.endsWith('.ngrok-free.dev')
    || hostname.endsWith('.ngrok-free.app')
    || hostname.endsWith('.ngrok.io')
}

// Determine what portion of `routePrefix` the proxy prepended for the current
// host. Returns '' when the browser URL is already the full internal path
// (dev direct access, or the `/org/otherOrg/...` fallback on the primary host
// which the proxy bypasses).
export function computeProxyStrip(hostname: string, routePrefix: string): string {
  if (isPrimaryHost(hostname)) {
    // Primary host: proxy only rewrites to `/org/${PRIMARY_ORG}`. Routes under
    // a different org (the no-custom-domain fallback) aren't rewritten, so
    // their URLs already show the full path — don't strip.
    const primary = `/org/${PRIMARY_ORG}`
    return routePrefix.startsWith(primary) ? primary : ''
  }
  // Custom host: the proxy always rewrote based on hostname resolution, so
  // the first tenant segment is exactly what was prepended.
  const m = routePrefix.match(/^(\/org\/[^/]+|\/[^/]+)/)
  return m ? m[1] : ''
}
...
export function PublicSiteLayout({
  ...
  // match what the browser actually shows. SSR returns the full prefix
  // (window unavailable) — the first client render corrects it.
  const getBasePrefix = () => {
    const full = routePrefix ?? `/${teacher.pageSlug}`
    if (typeof window === 'undefined') return full
    return stripPrefix(full, computeProxyStrip(window.location.hostname, full))
  }

  // Home/logo target. Falls back to '/' when stripping leaves an empty path
  // (the tenant root on its primary domain).
  const getHomeUrl = () => {
    const raw = homeUrl ?? routePrefix ?? `/${teacher.pageSlug}`
    if (typeof window === 'undefined') return raw || '/'
    return stripPrefix(raw, computeProxyStrip(window.location.hostname, raw)) || '/'
  }
