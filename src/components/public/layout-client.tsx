...
interface PublicSiteLayoutProps {
  ...
  routePrefix?: string // Server-side route prefix (e.g., '/org/eduskript/c' for org content, '/{pageSlug}' for teacher pages). Used to build navigation URLs; the proxy-stripped portion is removed at render time so the link matches the browser-visible path.
  ...
  proxyStripPrefix?: string // Prefix the proxy prepended server-side (e.g. '/org/eduskript' on eduskript.org, '/ig' on informatikgarten.ch). Stripped from all client-built URLs so they match the browser-visible path. Source: x-proxy-strip-prefix header set by src/proxy.ts. Undefined when no proxy rewrite happened (e.g. localhost direct path access).
...
export function PublicSiteLayout({
  ...
  // Base prefix for nav URLs. routePrefix is the full server-side path; we
  // strip whatever the proxy prepended so the resulting URL matches what the
  // browser actually shows (e.g. '/org/eduskript/c' → '/c' on eduskript.org).
  const getBasePrefix = () => stripPrefix(routePrefix ?? `/${teacher.pageSlug}`, proxyStripPrefix)

  // Home/logo target. Falls back to '/' when stripping leaves an empty path
  // (which happens at the org/teacher root on the primary domain).
  const getHomeUrl = () => stripPrefix(homeUrl ?? routePrefix ?? `/${teacher.pageSlug}`, proxyStripPrefix) || '/'
