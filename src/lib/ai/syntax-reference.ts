...
**Attributes (all optional):** \`id\` — keys the viewer's dismiss state (default: hash of the text, so new wording re-shows it); \`dismissible\` — \`false\` removes the close button; \`color\` — background: a palette name (\`paper\` = page background, \`muted\` = muted UI surface, or \`yellow|green|blue|pink|orange|red|purple\` translucent tints; all theme-aware) or any CSS color (default orange); \`text\` — text/link color: a text palette name (\`text\` = the page's normal text color, or \`red|blue|green|orange|purple|…\`; all theme-aware) or any CSS color (default white). Dismissal is stored per browser in localStorage.`)
...
**Attributes:** \`prompt\` — teacher instructions for the AI (not shown to students); \`solution\` — reference solution the AI compares against, never shown to students: an Excalidraw drawing name (as in \`<excali src>\`) or an image file from the skript, e.g. \`solution="kraefte-loesung"\` below a task drawing \`<excali src="kraefte-aufgabe" />\` that students draw on; \`id\` — optional stable identifier (components map to their prompt by position automatically, even with several per page); \`label\` — button text (default "Get AI feedback"). Place the tag after the exercise and the space students write in; with several tags under one h2, each sees everything above it (other tags' prompts excluded). Works for logged-out visitors too; requests are rate-limited per user or IP.`)
...
**Pricing:** \`<pricing lang="de" />\` — public price table (Free / Classroom / Supporter / School + "no traps" list) with live prices from the plan table; for landing pages. Put it on its own line; it has no signup button, add a \`<cta>\` after it if wanted.
